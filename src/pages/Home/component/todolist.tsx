...
import { Alert, Button, Card, Col, Input, List, Popover, DatePicker, Row, Switch, Tooltip, Form, FormInstance } from 'antd';
...
export default class TodoList extends Component<TodoProps>{    
  ...
  constructor(props: TodoProps | Readonly<TodoProps>){
    ...
    @action editToDoTitle = (index: number)=>{
      ...
      this.props.homePageStore.editToDo(index,{title, token:this.token, id:this.todoList[index].id});
      this.tmpEditValue = '';
    }
    ...
    @action editToDoContent = (index: number,values)=>{
      ...
      {
        console.log('edit content',values);
        this.props.homePageStore.editToDo(index,{token:this.token, id: this.todoList[index].id, start_time:values.timePicker[0], end_time:values.timePicker[1],description: values.description});
      }
    }
    ...
    render() {
        return(
            <Card title={<Alert message={'待办事项'} type="info" showIcon banner />}>
                ...
                name = 'todoInput'
                bordered = {false}
                style = {{marginBottom: 10,borderBottomColor:'#AECCF2', borderBottomWidth: 1,borderBottomStyle: 'solid'}}
                className = {styles.todoInput} placeholder = "回车添加事项"
                ...
                <List
                  ...
                  renderItem={(item,index) => (
                    <List.Item key={index} role={undefined} className="todoItem">
                      <Row gutter = {3}>                          
                        <Col span={24}>
                          <Checkbox style ={{marginTop:5,marginRight:5}} checked = {item.finished} onClick={this.finishToDo.bind(this,index)}/> 
                          <Input
                            style ={{width: 245, marginRight: 5}}
                            suffix={
                              <Tooltip title = {(item.description === undefined || item.description === '')?'暂无描述':item.description} style={{position:'absolute',right:0}}>
                                <Popover 
                                  title="请选择" trigger = 'click'
                                    ...
                                    >
                                      <Form.Item name = 'timePicker'>
                                        <RangePicker
                                          ...
                                          defaultPickerValue = {[moment(item.start_time),moment(item.end_time)]}
                                        />
                                      </Form.Item>
                                ...
                                >
                                  <Button shape='circle' size = 'small' icon={<EllipsisOutlined />} disabled= {item.finished}/>
                                </Popover>
                              </Tooltip>
                            }
                            disabled = {item.finished}
                            value={(this.isEditing && this.atEditing === index) ? this.tmpEditValue:item.title}
